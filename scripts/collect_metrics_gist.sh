...
LIMIT 50;
" > "$OUTDIR/03_worst_quality_scores.txt" 2>&1 || echo "(no quality_runs data)" > "$OUTDIR/03_worst_quality_scores.txt"
...
SELECT qr.doc_id, qm.metric_name, qm.metric_value, qm.passed,
       qm.threshold_value, qm.notes
FROM quality_metrics qm
JOIN quality_runs qr ON qm.run_id = qr.run_id
WHERE qm.passed = 0
ORDER BY qm.metric_name, qm.metric_value DESC
LIMIT 100;
" > "$OUTDIR/04_gate_failures.txt" 2>&1 || echo "(no quality_metrics data)" > "$OUTDIR/04_gate_failures.txt"
...
" > "$OUTDIR/05_escalation_failures.txt" 2>&1 || echo "(column not yet migrated — run import once first)" > "$OUTDIR/05_escalation_failures.txt"
...
if [ ${#NORMAL_FILES[@]} -gt 0 ]; then
    GIST_URL=$(gh gist create --public -d "predictor_ingest metrics $(date +%Y-%m-%d)" "${NORMAL_FILES[@]}")
