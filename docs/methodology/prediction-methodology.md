...
| A10 | **Methodology version stamp** (`config.METHODOLOGY_VERSION`): every `trend_history` row and every export `meta` block records the scoring-code version that produced it. Orthogonal to epoch (data boundary vs code boundary). Bump rule: increment in the same commit as any scoring-affecting change; `git log -S METHODOLOGY_VERSION` is the audit trail. "1" = Sprint-13-era scoring; "2" reserved for 20.7–20.9. Enables cheap cross-methodology comparison later (e.g. Track D retro-scores can be written under a distinct version and diffed in SQL) | §5/§8 (adds provenance both rely on) | shipped 2026-07-19 |
