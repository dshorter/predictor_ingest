...
| 18.5 | Auto-update via n8n webhook | GitHub push to `main` → `uzelhub.com/hooks/predictor` → Caddy reverse_proxy → n8n `/webhook/predictor-deploy` workflow → validate HMAC + check `ref == refs/heads/main` → `git -C /opt/predictor_prod pull --ff-only`. Reuses existing n8n infrastructure in `/opt/ai-agent-platform/`. Bind-mount `/opt/predictor_prod` into the n8n container so git runs inside the container without SSH key management. Add Caddy `handle_path /hooks/*` block. Add GitHub repo webhook with shared secret | [Sonnet] |
