...
| 18.5 | Auto-update via tiny deploy-hook service | Follows the existing house pattern (uzella-proxy). New tiny Node service under `services/predictor-deploy/` listens on `127.0.0.1:3002`, validates GitHub HMAC, checks `ref == refs/heads/main`, runs `git -C /opt/predictor_prod pull --ff-only`. systemd unit `predictor-deploy.service` runs the copy in `/opt/predictor_prod/services/predictor-deploy/` (self-updating with merges). New Caddy `handle_path /hooks/predictor` block reverse-proxies to `:3002`. Add GitHub repo webhook with shared secret. **Does NOT touch n8n** — Caddy + loopback Node service is the established pattern in `/opt/_host/README.md`; n8n is internal automation, not public-web infrastructure | [Sonnet] |
